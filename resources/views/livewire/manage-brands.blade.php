            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="w-full md:w-1/3">
                        <x-input wire:model.live.debounce.300ms="search" type="search" placeholder="Search brands..." class="w-full" />
                    </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($brands as $brand)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 truncate max-w-sm">{{ $brand->description ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="edit({{ $brand->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</button>
                                        <button wire:click="delete({{ $brand->id }})" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this brand?')">Delete</button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
