            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="w-full md:w-1/3">
                        <x-input wire:model.live.debounce.300ms="search" type="search" placeholder="Search customers..." class="w-full" />
                    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th wire:click="sortBy('phone')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100">
                                    Phone
                                </th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($customers as $customer)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 truncate max-w-xs" title="{{ $customer->address }}">{{ $customer->address ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if(!$customer->is_system)
                                            <button wire:click="edit({{ $customer->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3 font-semibold transition">Edit</button>
                                            <button wire:click="delete({{ $customer->id }})" class="text-red-600 hover:text-red-900 font-semibold transition" onclick="return confirm('Are you sure you want to delete this customer?')">Delete</button>
                                        @else
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mt-4 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="address" wire:model="address" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"></textarea>
